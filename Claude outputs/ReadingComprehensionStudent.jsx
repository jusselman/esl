...
export default function ReadingComprehensionStudent() {
  ...
  const isActivityStarted = !!startedAt

  // Show waiting screen until activity is launched by teacher
      ...
      <div className={styles.root}>
        <div className={styles.waitingContainer}>
          <div className={styles.waitingContent}>
            <h2>You're in!</h2>
            <p>Waiting for the teacher to start the activity...</p>
  ...
  // Determine target word count based on time limit (in minutes)
  let targetWords = 20
  if (timeLimit === 5) targetWords = 100
  if (timeLimit === 10) targetWords = 200

  // Calculate timer values (timeLimit is in minutes)
  const startedAt = new Date(activityState.startedAt).getTime()
  const endsAt = startedAt + (timeLimit * 60 * 1000)
