...
import {
  ...
  startSynonymDuel,
  advanceSynonymQuestion,
  buildSynonymLeaderboard,
  AVATARS,
} from '../store/gameStore'
import styles from './SynonymsHost.module.css'

const CHOICE_LETTERS = ['A', 'B', 'C', 'D']

export default function SynonymsHost() {
  ...
  useEffect(() => {
    ...
    // synonym_answers (same fallback used for grammar_answers).
  ...
  async function handleBeginActivity() {
    ...
    setStarting(true)
    try {
      await startSynonymDuel(roomCode)
    } finally {
      setStarting(false)
    }
  }
  ...
  async function handleNext() {
    ...
    setAdvancing(true)
    try {
      await advanceSynonymQuestion(roomCode)
    } finally {
      setAdvancing(false)
    }
  }
    ...
    return (
      <div className={styles.root}>
        ...
        <header className={styles.header}>
          <div className={styles.activityTag}>Synonyms — {tierEmoji} {tierTitle}</div>
        </header>
  ...
  if (!currentQuestion) {
    return (
      <div className={styles.root}>
        <div className={styles.loading}>Loading word...</div>
      </div>
    )
  }
  ...
  return (
    <div className={styles.root}>
      ...
      <div className={styles.questionCard}>
        ...
        <h2 className={styles.questionPrompt}>{currentQuestion.clue}</h2>
      ...
      {timeUp && (
        <div className={styles.revealFooter}>
          ...
          <button className={styles.primaryBtn} onClick={handleNext} disabled={advancing}>
            {advancing ? 'Loading…' : isLastQuestion ? 'Show Final Results' : 'Next Word'}
          </button>
        </div>
      )}
    </div>
